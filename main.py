...
class CryptoSimApp:
    ...
    def init_dashboard(self, username):
        ...
        def switch_tab(tab_name):
            ...
            tk.Label(content, text=tab_name, font=self.title_font, bg="#f3f3f3").pack(pady=20)

            if tab_name == "Homepage":
                welcome_frame = tk.Frame(content, bg="white", bd=1, relief="groove")
                welcome_frame.pack(padx=20, pady=(20, 10), fill="x")
                tk.Label(welcome_frame, text=f"Welcome, {username}", font=("Helvetica", 14), fg="#1c1c1c", bg="white").pack(padx=10, pady=10, anchor="w")

                card_frame = tk.Frame(content, bg="white", bd=1, relief="groove")
                card_frame.pack(padx=20, pady=10, fill="x")

                card_image = tk.Label(card_frame, text="💳", font=("Helvetica", 20), bg="white")
                card_image.grid(row=0, column=0, rowspan=2, padx=10, pady=10)

                tk.Label(card_frame, text="My Balance", font=self.text_font, fg="gray", bg="white").grid(row=0, column=1, sticky="w")
                tk.Label(card_frame, text="$634.22 AUD", font=("Helvetica", 16, "bold"), fg="#1c1c1c", bg="white").grid(row=1, column=1, sticky="w")

                tk.Label(card_frame, text="▲ 9.11% Today", font=("Helvetica", 10), fg="green", bg="white").grid(row=1, column=2, sticky="e", padx=10)

                deposit_btn = tk.Button(card_frame, text="+ Deposit", bg="black", fg="white", font=("Helvetica", 10), relief="flat")
